...
const server = http.createServer(function (req, res) {
    ...
    if (req.method == 'GET') {
        fs.readFile('knjige.txt', function (err, data) {
            ...
            for (let i = 0; i < redovi.length; i++) {
                ...
                let objekat = {naslov: kolone[0], cijena: kolone[1], tip: kolone[2], ocjena: kolone[3]};
                niz.push(objekat);
            }
    ...
    if (req.method == 'POST') {
        ...
        req.on('end', function () {
            ...
            novaLinija += '\n' + parametri.get('naslov') + ',' +
                ...
                parametri.get('tip') + ',' + parametri.get('ocjena');
    ...
    if (req.method == 'DELETE')
    {
        ...
        req.on('end', function () {
            let parametri = new url.URLSearchParams(tijeloZahtjeva);
            let novaLinija = '';
            fs.writeFile('knjige.txt', novaLinija, function (err)
            {
                if (err) throw err;
                console.log();
                res.writeHead(200, {});
